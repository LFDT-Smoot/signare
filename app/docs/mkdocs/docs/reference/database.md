...
**Do not recover by rolling back to a release older than the one that introduced pin sources.** The column is gone by then, so the older mappers, which select it by name, fail on every slot read including a module-wide listing. Reverting migration `000005` does not help either: it restores the column, but empty, so no slot has a PIN to sign with. Running the older `signare upgrade` afterwards panics either way: with the dirty flag still set it refuses the dirty version, and with the flag cleared it sees a version beyond its own step list, takes the downgrade branch and panics on an out-of-range step. Recover forward, with the steps below.
