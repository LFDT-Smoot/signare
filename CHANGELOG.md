...
- Run `rbac-validator` from a `make tools.validate_rbac` target, and read its operation ID exemption list from `actions-manual.yaml` instead of a copy of that list in the tool's Makefile, with the RBAC coverage tests pinning that file to the published JSON-RPC method set in both directions (#28).
