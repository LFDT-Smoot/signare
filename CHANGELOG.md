...
### Security
- Bind loopback by default and warn when told not to: `--listen-address` now defaults to `127.0.0.1` and also governs the Prometheus metrics listener, which previously bound every interface regardless, and a non-loopback bind logs a startup warning stating the proxy contract the identity headers depend on. Breaking for published-port containers and off-host scrapers, both restored with `--listen-address 0.0.0.0` (#47).
