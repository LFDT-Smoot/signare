...
- Every JSON-RPC method now reads the positional array param form and the single object form identically. The array form previously matched field names exactly, so a request valid as an object was rejected as an array. It also required `data` on `eth_signTransaction`, which is now optional in both forms, as the Ethereum specification has it. The EIP-1559 fee ordering check and a new requirement that every `accessList` and `authorizationList` entry carry an `address` now apply to both forms; the array form no longer requires an entry to spell out `storageKeys`, since omitting it and passing an empty list mean the same thing (#40).
