...
- Reject a JSON-RPC params object that names the same field twice under different casing, before anything decodes it. Per-account authorization decoded the params into a struct, where `encoding/json` matches field names case-insensitively and keeps the last match, while the signing path read an exact key out of a `map[string]any`. For the positional array form the two therefore disagreed, and params carrying both `from` and `From` were authorized against one account and signed with the other, letting a caller authorized for any one account in an application sign with any other account in that application's slot. This affected `eth_signTransaction` and `personal_sign`; `eth_signTypedData` already decoded both param forms the same way on both paths. Authorization now resolves the params through the same unwrap the handler uses, so the two cannot read a different account out of one request (#40).
