...
### Security
- Reject EIP-712 typed data whose type definitions are cyclic, and bound the depth and total work of the encoding walk, closing a denial of service where a crafted message either crashed the signer with an unrecoverable stack overflow or expanded to effectively unbounded work. A malformed array type string now returns an error instead of panicking, and a self-referencing type that previously encoded only because its array happened to be empty is now rejected up front (#13).
