...
### Security
- **Breaking:** `--listen-address` now defaults to `127.0.0.1` and also governs the Prometheus metrics listener, which previously bound every interface regardless; a non-loopback bind logs a startup warning stating the proxy contract the identity headers depend on, and an IPv6 bind address is now bracketed so it reaches the listener at all. Published-port containers and off-host scrapers need `--listen-address 0.0.0.0` set explicitly (#47).
