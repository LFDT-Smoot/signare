...
- Correct two documented defaults that matched nothing in the code: the Prometheus metrics port, given as 9780 in the configuration reference and in two Go doc comments where `defaultPrometheusPort` is 9785, and the OpenAPI spec's `servers` URL, given as `http://localhost:34343` where the REST listener defaults to 32325 (#47).
