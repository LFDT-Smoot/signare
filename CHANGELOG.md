...
- `eth_signTransaction` and `personal_sign` now read the positional array param form and the single object form identically. The array form previously matched field names exactly and required `data`, so a request valid as an object was rejected as an array. `data` is optional in both, and the EIP-1559 fee ordering and access list entry checks now apply to both (#40).
...
- Reject a JSON-RPC params object that names the same field twice under different casing. Per-account authorization and the signing path decoded the positional array form differently, so params carrying both `from` and `From` were authorized against one account and signed with the other, letting a caller authorized for one account in an application sign with any other account in that application's slot. Affected `eth_signTransaction` and `personal_sign` (#40).
